# Discussione Narrativa — FuoriDaHellfire

Todo list e note per le scelte narrative.

---

## L'Anello del Virtuoso — SCELTA DEFINITIVA

### L'oggetto

Un **anello d'argento annerito** con un'incisione interna: una chiave di violino che si morde la coda (un ouroboros musicale). L'anello è al dito di Korex — è l'origine del suo potere.

### Backstory

L'anello è passato di portatore in portatore per secoli, da quando l'Ordine della Chiave Spezzata vi ha imprigionato Jason Accordion. Korex l'ha ottenuto da un precedente portatore (come e dove è un punto aperto della saga). L'ha infilato, e il potere è arrivato. Ma l'anello non si toglie. Korex ha smesso di provarci quando ha capito quanto era forte. L'anello contiene l'anima di un bardo antico che vuole tornare in vita.

Riferimento: Robert Johnson al crossroads — Korex ha "venduto l'anima" infilandolo, non al diavolo, ma a un altro bardo che voleva una seconda vita.

### Meccanica a tre livelli

**Livello 1 — Attrazione (passiva)**
Quando Korex muore o è incapacitato/imprigionato, l'anello inizia ad attirare debolmente. Chiunque lo veda sente un impulso sottile a prenderlo — non un charm, più una curiosità insistente. Non richiede tiro salvezza, è narrativo.

**Livello 2 — Tocco (attivazione)**
Chi tocca l'anello (raccogliendolo dal dito di Korex, esaminandolo, ecc.) sente un calore che sale dal dito al braccio. Una risata bassa risuona nelle fogne. La voce: *"Libero. Finalmente libero."*

Il PG deve superare un **TS Saggezza CD 20**. Se fallisce, è obbligato a indossare l'anello immediatamente. Se riesce, riesce a posarlo — ma l'attrazione resta.

**Livello 3 — Indossato (maledizione)**
L'anello si stringe e non si toglie più.

- **Vantaggio**: +2 a una caratteristica (per classe del portatore, vedi tabella in `OBJ_AnelloDelVirtuoso.md`) + 1 incantesimo da bardo 1/giorno senza slot + vantaggio TS contro charm.
- **Maledizione**: possessione progressiva (meccanica Presa). Vedi cheat sheet sotto per dettagli.

### Cliffhanger

> Appena l'anello scivola al dito, il mondo si ferma per un istante. Un calore sale dal dito, al braccio, al petto. Poi una risata — bassa, profonda, soddisfatta — risuona nelle fogne. Non viene dall'anello. Viene da ovunque.
>
> *"Libero. Finalmente libero."*
>
> L'anello si stringe. Non si toglie più. E per un istante — solo un istante — vedete qualcosa nei vostri occhi riflessi in una pozza d'acqua. Non il vostro volto. Quello di qualcun altro. Che sorride.

### Piano B — Nessun PG lo tocca

Se nessun PG tocca l'anello, viene consegnato alla Guardia di Oakshore con il resto del bottino. Un NPC guardia lo indossa (attirato dal Livello 1). Nella sessione successiva o nel cliffhanger stesso:

> Tornate alla superficie, consegnate Korex e il bottino. Il capitano della Guardia vi paga. Tutto sembra finito. Ma mentre vi allontanate dai moli, sentite un urlo dietro di voi. Vi girate: una delle guardie è in piedi, immobile, con l'anello al dito. I suoi occhi sono diversi. Sorride — un sorriso che non è il suo.
>
> *"Libero."*
>
> La guardia estrae la spada.

Questo lancia la prossima avventura con un combattimento immediato e un mistero: cos'è successo alla guardia?

### Semi per la saga (lv4-6)

- Chi era il bardo nell'anello? Perché era nelle fogne antiche?
- La corsa contro il tempo per rimuovere l'anello prima che il bardo prenda il sopravvento
- Il bardo ha un piano proprio — non è un alleato, ha obiettivi che si rivelano progressivamente
- Le fogne antiche: chi le ha costruite? Cosa c'è più in profondità?

---

## Decisioni prese

- [x] Variante: Anello del Virtuoso
- [x] Meccanica: tocco → TS Sag CD 20 → indossamento forzato
- [x] Attrazione passiva quando Korex cade
- [x] Piano B: guardia NPC lo indossa → attacco → cliffhanger
- [x] Voce: "Libero. Finalmente libero." (= nuovo ospite senza difese, non uscita dall'anello)
- [x] L'anello amplifica ciò che il portatore ha, non ha poteri fissi
- [x] Possessione dipende dalla resistenza attiva del portatore, non solo dal tempo
- [x] Maledizione subito evidente (anello non si toglie + incubi prima notte + freddo costante)
- [x] Korex resisteva parzialmente a Jason — per questo era ancora sé stesso

## Punti aperti — da approfondire

- [ ] **Piano B — cliffhanger alternativo**: dopo la guardia posseduta, l'anello resta. I PG non sanno come eliminare il pericolo. La saga = come disfarsi dell'anello.
- [ ] Come ha ottenuto Korex l'anello? (da chi, dove — non nelle fogne di Oakshore)
- [ ] Dove è lo scheletro di Jason? (non nelle fogne — trovarlo è la vera quest della saga)
- [ ] La catena dei portatori: quanti? Tracce/storie dei precedenti come indizi?
- [ ] A che livello di presa la possessione diventa permanente e irreversibile?
- [ ] Livello esatto del caster per il rituale salt-and-burn, e se è un NPC chi è?

---

## Cheat Sheet — L'Anello del Virtuoso (riferimento rapido DM)

**Aspetto**: anello d'argento annerito, incisione ouroboros musicale. È al dito di Korex.

### Attivazione — 3 livelli

1. **Attrazione** (passiva, narrativa): quando Korex cade, chiunque veda l'anello sente curiosità. Nessun tiro.
2. **Tocco**: chi tocca l'anello → TS Saggezza CD 20. Fallimento = obbligato a indossarlo. Successo = riesce a posarlo, attrazione resta.
3. **Indossato**: l'anello si stringe, non si toglie. Sintonia automatica.

### Effetti immediati

| vantaggi | malus |
|----------|-------|
| +2 a una caratteristica (vedi tabella per classe in `NPC_JasonAccordion.md`) | L'anello non si toglie |
| 1 incantesimo da bardo 1/giorno senza slot | Incubi la prima notte (niente beneficio riposo lungo) |
| Vantaggio TS contro charm | Freddo costante alla mano |
| | Vulnerabilità danni psichici (progressiva) |

### Possessione — tracciamento presa

Ogni uso dell'incantesimo 1/giorno → tira **d8** ("per vedere se funziona").
- **2-8**: funziona.
- **1**: fallisce + onda psichica 1d4+presa a tutti entro 10qd + presa +1.

| presa | narrativo | meccanico |
|-------|-----------|-----------|
| 0 | Freddo, incubi | Vulnerabilità psichica |
| 1-2 | Tic di Jason, sussurri rari | Onda 1d4+1/+2 |
| 3-4 | Jason parla spesso, ricordi altrui | Onda 1d4+3/+4, TS Sag CD 14 in stress |
| 5-6 | Jason prende il controllo 1d4 round | Onda 1d4+5/+6, TS Sag CD 15 |
| 7+ | Tentativo controllo permanente | Onda 1d4+7+, TS Sag CD 16+ |

**Ridurre presa**: TS Saggezza riuscito quando Jason forza conflitto (−1), *Lesser Restoration* (−1), *Greater Restoration* (−2).

### Rimozione

*Remove Curse* allenta l'anello per 1d4 ore, poi torna. Rimozione definitiva: trovare lo scheletro di Jason + bruciarlo con sale + mago/ordine che conosce il rituale originale. Dettagli in `NPC_JasonAccordion.md`.

### Piano A vs Piano B

- **Piano A** (PG tocca l'anello): possessione progressiva del PG. Saga = salvare il compagno.
- **Piano B** (nessun PG lo tocca): una guardia lo indossa → attacco immediato → i PG la sconfiggono → l'anello resta. Saga = come disfarsi dell'anello.
- **Piano B variante** (Fin lo tocca): Fin ha SAG 10, Jason lo possiede più facilmente. Saga = salvare l'amico.

### "Libero"

Jason dice "Libero" perché ha un nuovo ospite senza difese — non perché è uscito dall'anello. Korex sapeva resistergli. Il nuovo portatore no.
