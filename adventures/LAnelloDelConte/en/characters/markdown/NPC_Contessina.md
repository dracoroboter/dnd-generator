> ⚠️ Auto-translated from Italian. The Italian version is the source of truth.

# NPC_Contessina - The Countess

## General Information

- **Full name**: La Contessina (the Count's wife)
- **Role**: noblewoman, wife of the Count of Fianus
- **Race**: Human
- **Alignment**: Lawful Neutral

## Description

A stout woman with a round face and a permanently indignant expression. She dresses well but without taste - too many jewels, too many colors. She speaks with a thick accent and gesticulates a lot. She holds down the castle in Aviano while the Count is "traveling" (i.e., always). She knows about the blackmail but her main concern is her own reputation, not her husband.

## Stat Block

> Base: **Noble** (Monster Manual p. 348, CR 1/8)

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 11 (+0) | 12 (+1) | 11 (+0) | 12 (+1) | 14 (+2) | 16 (+3) |

- **Hit Points**: 9 (2d8)
- **Armor Class**: 15 (breastplate)
- **Speed**: 9m / 30ft / 6sq
- **Skills**: Insight +4, Persuasion +5, Deception +5
- **Senses**: passive Perception 12
- **Languages**: Common
- **Challenge**: 1/8 (25 XP)

## Notable Abilities

- **Parry**: The Countess adds 2 to her AC against one melee attack that would hit her. She must see the attacker and be wielding a melee weapon.

## Attacks

### Rapier (melee)
- **Attack**: +3, melee 5ft
- **Damage**: 1d8+1 piercing

## Role in the Adventure

Narrative NPC in Episode 3. She knows about the blackmail (the compromising portrait) because Gorim informed her. She's embarrassed and worried about her reputation. She doesn't know who S is. Accessible only with Gorim's letter/seal.

## DM Notes

- She doesn't fight - the Noble stat block is for completeness.
- Her concern is reputation, not the Count.
- If PCs ask about the Count: "He's traveling. He's ALWAYS traveling."
- She won't admit knowing about the Count's "business dealings."
- Castle access: requires Gorim's letter or 1 sp each for the guards.
