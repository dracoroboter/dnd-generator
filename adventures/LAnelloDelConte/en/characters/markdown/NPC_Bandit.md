> ⚠️ Auto-translated from Italian. The Italian version is the source of truth.

# NPC_Bandit — bandit

## General Information

- **Full name**: Bandit
- **Role**: Er Braccio's henchman
- **Race**: Human
- **Alignment**: Chaotic Neutral

## Description

Port men of Vetusta Portus. Dockworkers, ex-sailors, petty criminals. They work for Er Braccio for easy money. They're not fanatics — if things go south, they run.

## Stat Block

> Base: **Bandit** (Monster Manual p. 343, CR 1/8)

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 11 (+0) | 12 (+1) | 12 (+1) | 10 (+0) | 10 (+0) | 10 (+0) |

- **Hit Points**: 11 (2d8+2)
- **Armor Class**: 12 (leather armor)
- **Speed**: 9m / 30ft / 6sq
- **Senses**: passive Perception 10
- **Languages**: Common
- **Challenge**: 1/8 (25 XP)

## Attacks

### Scimitar (melee)
- **Attack**: +3, melee 5ft
- **Damage**: 1d6+1 slashing

### Light Crossbow (ranged)
- **Attack**: +3, ranged 80/320 ft.
- **Damage**: 1d8+1 piercing

## Role in the Adventure

4 bandits in Warehouse 4 of Vetusta Portus (Episode 3). Two guarding the main door, two hidden among the crates inside. They fight for 1-2 rounds, then flee if Er Braccio escapes.

## DM Notes

- Identical stat blocks for all 4.
- They don't fight to the death.
- The two outside guards arrive at round 2 if not already eliminated.
