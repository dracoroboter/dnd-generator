...
def render(data, tile, output_path, seed):
    ...
    for group, orient in door_groups:
        ...
        if orient == 'h':
            # Muro orizzontale → denti verticali (su/giù)
            ...
            # Muro verticale → denti orizzontali (sinistra/destra)
    ...
    # ── Enrichment (finestre e oggetti) ──────────────────────────────────────
    enr = data.get('_enrichment', {})

    # Finestre: trattino corto sul muro esterno
    # Regola: non sovrapporre a porte esistenti
    door_positions = set((d['x'], d['y']) for d in data.get('doors', []))
    ...
    for win in enr.get('windows', []):
        ...
        # Calcola le celle del muro corrispondente e verifica assenza porte
        if wall == 'bottom':
    ...
    # Oggetti: 1 quadretto (tile×tile), snappato alla griglia
    for obj in enr.get('objects', []):
        room = next((r for r in rooms if r['id'] == obj['room']), None)
        if not room:
            continue
        rx2, ry2, rw, rh = room['x'], room['y'], room['w'], room['h']
        # Snap al quadretto più vicino
        gx = rx2 + round(obj['x_rel'] * rw - 0.5)
        gy = ry2 + round(obj['y_rel'] * rh - 0.5)
        gx = max(rx2, min(rx2+rw-1, gx))
        gy = max(ry2, min(ry2+rh-1, gy))
        ox = (gx - x0) * tile
        oy = (gy - y0) * tile
        t = obj.get('type', 'chest')
        if t == 'chest':
            L.append(f'<rect x="{ox}" y="{oy}" width="{tile}" height="{tile}" fill="white" stroke="black" stroke-width="1"/>')
            L.append(f'<rect x="{ox}" y="{oy}" width="{tile}" height="{tile//2}" fill="white" stroke="black" stroke-width="1"/>')
            L.append(f'<line x1="{ox}" y1="{oy+tile//2}" x2="{ox+tile}" y2="{oy+tile//2}" stroke="black" stroke-width="1"/>')
            L.append(f'<circle cx="{ox+tile//2}" cy="{oy+tile//2}" r="{max(1.5, tile//8)}" fill="black"/>')
