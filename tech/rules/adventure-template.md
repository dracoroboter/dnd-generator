...
**Convenzione directory:** `NN_` con numero progressivo (01_, 02_, ...). Le directory `XX*_` (es. `XX3_IlFinaleDiStagione/`) sono bozze di moduli futuri non ancora numerati — escluse dai check di normalizzazione e dalla pubblicazione (non finiscono nel PDF).
